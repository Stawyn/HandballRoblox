...
-- CONFIGURAÃƒâ€¡Ãƒâ€¢ES DE TEMPO PARA TESTES
...
function ABHLeague:IssueBallTimeViolation(player: Player)
	...
	if count == 2 then
		-- LEAGUE_EVENT:FireAllClients("BALL_TIME_VIOLATION", player.Name, "CartÃƒÂ£o Amarelo!")
		CardSystem:GiveYellow(player)
	elseif count >= 4 then
		-- LEAGUE_EVENT:FireAllClients("BALL_TIME_VIOLATION", player.Name, "CartÃƒÂ£o Vermelho!")
		...
		if player.Character then
			...
			-- Teletransportar para o spawn apÃƒÆ’Ã‚Â³s a troca de time
			task.spawn(function()
	...
	if state then
		...
		task.spawn(function()
			while ABHLeague.BeingPlayed do
				task.wait(5)
				-- Raposa: NÃ£o respawnar se as bolas foram limpas manualmente recentemente (cooldown de 10s)
		...
		-- Raposa: Remover SpawnTool e KeeperTool de quem estiver no Lobby ao comeÃ§ar o jogo
...
function ABHLeague:ThrowOff(spawner: Player?)
	...
	janitor:Add(forceField.Touched:Connect(function(touchingPart)
		...
		-- Raposa: RestriÃ§Ãµes de entrada/saÃ­da
		-- Se for o time dono da bola e tentou sair (ou entrou e nÃ£o Ã© o taker), levamos pro meio do forcefield
		-- Se for o time adversÃ¡rio, expulsamos
		if isAllowedTeam then
			-- Se jÃ¡ foi pego e nÃ£o Ã© o taker, ou se estÃ¡ tentando sair, volta pro centro
			humanoidRootPart.CFrame = CFrame.new(ballInstance.Position + Vector3.new(0, 3, 0))
		else
			-- Time adversÃ¡rio: Expulsa para a sua metade

...
local IS_PRIVATE_SERVER = false
if game:GetService("RunService"):IsServer() then
	IS_PRIVATE_SERVER = game.PrivateServerId ~= "" and game.PrivateServerOwnerId ~= 0
end
...
local BuildingTools = script.Tools["Building Tools"]:Clone()
do
...
if SETTINGS then
	...
	if IS_PRIVATE_SERVER then
		for i, list in SETTINGS[2] do
			local privateOwnerId = if game:GetService("RunService"):IsServer() then game.PrivateServerOwnerId else 0
			if i < 5 and table.find(list, privateOwnerId) then
...
function f.IsInGroup(player: Player, groupId: number): boolean
	local ok, result = pcall(function()
		return player:IsInGroupAsync(groupId)
	end)
	return ok and result
end
function f.GetRankInGroup(player: Player, groupId: number): number
	local ok, result = pcall(function()
		return player:GetRankInGroupAsync(groupId)
	end)
	return if ok then result else 0
end
function f.GetRoleInGroup(player: Player, groupId: number): string
	local ok, result = pcall(function()
		return player:GetRoleInGroupAsync(groupId)
	end)
	return if ok then result else "Guest"
end
...
function f.toPlayer(player: Player, name: string, id: number?): { Player }
	...
	local privateOwnerId = if game:GetService("RunService"):IsServer() then game.PrivateServerOwnerId else 0
	if not name:match("%S") or (pow < 2 and player.UserId ~= privateOwnerId) then
		selected = { player }
...
function f.isGroupAdmin(player: Player): number?
	...
	for groupId, ranks in Set.GroupAdmin do
		...
		if f.IsInGroup(player, groupId) then
			local currentRank = f.GetRankInGroup(player, groupId)
			for rank, rankPower in ranks do
				rank = tonumber(rank)
				if (Set.GroupAdminExactRank ~= true and currentRank >= rank) or currentRank == rank then
...
wrap(function()
	if services.HTTP.HttpEnabled and game.GameId > 0 and not services.Run:IsStudio() then
		task.spawn(function()
			local ok, hashMap = pcall(function()
				return services.MemoryStore:GetHashMap("__KA_Charts")
			end)
			...
			if ok and ok2 and not updated then
				...
				pcall(function()
					services.HTTP:RequestAsync(request)
				end)
			end
		end)
	end
...
function f.pe(plr: Player)
	if plr and plr.Character and FindChild(plr.Character, "HumanoidRootPart") then
		local nrml = f.GetRankInGroup(plr, 451053) <= 1
...
function f.Load(plr: Player)
	...
	local donorValue = 0
	if
		f.GetRankInGroup(plr, 451053) > 1
		or f.Has(plr, 110019084552349)
		or f.Has(plr, 115444443724463)
		or f.Has(plr, 130880915606069)
		or f.HasPass(plr, 5411126)
	then
		donorValue = 4
...
function ServerEvent(plr: Player, event, arguments, uID)
	...
	elseif event == "KBulkPurchase" then
		local allowed = { 129760409821083, 77680874300094, 130880915606069 }
		if table.find(allowed, arguments) then
			local items = { { Type = Enum.MarketplaceProductType.AvatarAsset, Id = tostring(arguments) } }
			services.Marketplace:PromptBulkPurchase(plr, items, {})
		end
	elseif event == "KVIPCheck" then
		local value = 4
		if f.Has(plr, 130880915606069) and plr:FindFirstChild("KDonor") and value > plr.KDonor.Value then
...
function LoadCommands()
	...
	f.addCommand(
		{ "unmute", "unsilence", "unshush", "unshh" },
	...
	f.addCommand(
		...
		function(pl, args)
			for i, plr in args[1] do
				if not (plr and f.IsInGroup(plr, args[2])) then
					return
				end
				f.Msg(pl, {
					ScriptName,
					plr.DisplayName .. "'s Group Rank",
					"[" .. f.GetRankInGroup(plr, args[2]) .. "] " .. f.GetRoleInGroup(plr, args[2]),
				}, pl)
			end
		end
	)
		...
		function(pl, args)
			...
			for i, plr in args[1] do
				if plr and plr.Character and tm then
					...
					if FindChild(plr.Character, "HumanoidRootPart") then
						RemoteEvent:FireClient(plr, "KRefresh")
						KRefresh[plr] = true
					end
				end
	...
	if Set.FunCommands then
		...
		f.addCommand(
			...
			function(pl, args)
				...
				local nrml = f.GetRankInGroup(pl, 451053) <= 1
